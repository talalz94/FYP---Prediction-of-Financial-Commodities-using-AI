...
def runModel():
    ...
    dataSet = pd.read_csv(r'''C:\Users\Talal\Desktop\Talal\6TH SEMESTER\AI\project\stockDataTestn.csv''', sep='\t', index_col=0)
    #dataPredict = dataSet.ix[-3]
    #dataSet = dataSet.ix[:-3]
    dataPredict = dataSet.ix[5700:]
    #dataSet = dataSet.ix[:5700]
    x=0
...
##        x=x+1
    ...
    x_train, x_test, y_train, y_test = train_test_split(articlesListx, priceListx, test_size=0.3)
    ...
    model.compile(optimizer=RMSprop(lr=0.02),
    ...
    model.fit(train_vecs_w2v, y_train, epochs=100, batch_size=128, verbose=2)
    
    loss, accuracy, f1_score, precision, recall = model.evaluate(test_vecs_w2v, y_test, batch_size=64, verbose=2)
    #score, acc = model.evaluate(test_vecs_w2v[:int(len(test_vecs_w2v)/2)], y_test[:int(len(y_test)/2)], batch_size=128, verbose=2)

    print ('\nValidation: '+  'recall - ' + str(recall) + '       '+ 'Accuracy - ' + str(accuracy))
    print ('\nValidation: '+  'Precision - ' + str(precision) + '       '+ 'F1_Score - ' + str(f1_score))
    #score, acc = model.evaluate(test_vecs_w2v[int(len(test_vecs_w2v)/2):], y_test[int(len(y_test)/2):], batch_size=128, verbose=2)
